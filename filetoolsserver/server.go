...
- backup_store: optional persistent-store status/list/inspect/audit plus original-target restorePreview/restoreApply; restore uses one-shot capabilities, exact verified object bytes, a mandatory durable safety backup for an existing target, and missing-target no-replace; no object bytes, internal paths, or automatic rollback are exposed
