...
- Extended the existing 27th `backup_store` tool rather than adding a new tool. Its catalog annotations are now non-read-only and destructive because restore may replace an authorized original target; status/list/inspect/audit remain individually read-only. Restore plans are fixed-cap bounded, use the configured plan TTL, omit unsafe/oversized diffs, and remain transport-independent across stdio and stateful Streamable HTTP.
