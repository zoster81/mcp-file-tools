...
- Added `MCP_BACKUP_STORE_DIR` plus bounded total-byte, object-byte, manifest, per-target-version, pinned, retention, and plan-lifetime configuration with approved defaults and hard maxima. Configuring the store exposes no backup-management action and does not automatically capture mutations.
...
- Expanded the current unreleased source catalog from 23 to 26 tools while preserving identical stdio and Streamable HTTP schemas and behavior.
