...
R17 approved the persistent-backup lifecycle, and R18 phases 1–5 are implemented in source. An operator may configure `MCP_BACKUP_STORE_DIR` to initialize a separate non-overlapping internal store with owner-only permissions, one lifetime writer lock, an immutable descriptor, bounded recovery, and a rebuildable index. Internal code captures exact bytes into verified SHA-256 objects and strict checksummed manifests under conservative quotas. The read-only `backup_store` tool exposes redacted status, authorization-filtered newest-first listing, fully verified metadata inspection, and bounded quick/full audits over both transports. `edit_file` preview and `patch_package` manifests may bind `backupPolicy: "required"`. Edit apply captures its approved pre-state before mutation; package dry run preflights one conservative all-target reservation, and package apply captures every changed pre-state before the first commit. Outputs return durable backup identifiers. Policy omission, direct editing, and logical no-ops still create no persistent backup. Restore, garbage collection, mutable pinning, and automatic rollback remain unavailable.
...
- **optional backup store:** `MCP_BACKUP_STORE_DIR` must be a separate canonical non-overlapping path and is denied to ordinary file tools; `backup_store` exposes only bounded read-only metadata, while `edit_file` can invoke capture only through an approved preview carrying `backupPolicy: "required"`.
...
| `MCP_BACKUP_STORE_DIR` | Unreleased R18 dedicated internal store. Must be absolute, canonical, non-overlapping with public roots, owner-only, and exclusively lockable. Startup performs bounded recovery and index validation; no MCP or normal mutation creates backups yet. | unset |
...
| `MCP_BACKUP_PLAN_TTL_SECONDS` | Reserved future restore/GC one-shot plan lifetime. Hard maximum: 86,400 seconds. | `900` |
