...
- Active development milestone: R18 persistent backups; phases 1–5 provide the protected store, exact-byte capture/recovery/index/audit core, the read-only `backup_store` tool, approval-bound `edit_file`, and conservative all-target required `patch_package` capture while the unreleased catalog remains 27 tools and the published runtime remains unchanged
