...
- Active development milestone: R18 persistent backups; phases 1–6 provide the protected store, exact-byte capture/recovery/index/audit core, bounded `backup_store` review plus original-target restore, approval-bound `edit_file`, and conservative all-target required `patch_package` capture while the unreleased catalog remains 27 tools and the published runtime remains unchanged
